...
class headerexhortation extends CI_Controller {
    ...
    public function do_upload() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->view('updates/headingexhortation', $data);
                ...
                if (!$this->upload->do_upload('userfile')) {
                    ...
                    $data["msg"] = $this->headermodel->updateexhortation($rec);
        ...
        public function load() {
            if ($this->session->userdata("admin_pass") == "")
                redirect("welcome/");
            $db= $this->headermodel->getexhortation();
            $church= $this->churchinfo->getchurchinformation();
            $pic=$this->footerbackg->getfooterbg();
            $data["church"]=$church["name"];
            $data["favicon"]=$pic["photo"];
