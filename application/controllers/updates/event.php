...
class event extends CI_Controller {
    ...
    public function do_upload() {
        ...
        $this->form_validation->set_rules("time", "Time", "required|trim|min_length[3]");
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!$this->upload->do_upload('userfile')) {
                ...
                $rec["info"] = $this->input->post("summary");
                $rec["speaker"] = $this->input->post("speaker");
                $rec["role"] = $this->input->post("role");
                $rec["title"] = $this->input->post("title");
                $rec["time"] = $this->input->post("time");
